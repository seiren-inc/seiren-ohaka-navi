    ...
    return (
        ...
        >
            ...
            >
                ...
                <button
                    onClick={onClose}
                    className="absolute right-4 top-4 p-2 bg-bg hover:bg-border rounded-full transition-colors z-10"
                    aria-label="閉じる"
                >
                ...
                <div className="p-6 border-b border-border bg-bg/50">
                    ...
                    <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-sm font-bold text-text-secondary">円</span>
                        <span className="text-xs text-text-muted ml-2">（総額目安）</span>
                    </div>
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                    {/* Left: Image */}
                    <div>
                        <div className="aspect-video w-full bg-border rounded-lg flex items-center justify-center text-text-muted mb-4 overflow-hidden relative">
                        ...
                        <div className="bg-bg rounded-lg border border-border overflow-hidden">
                            <dl className="divide-y divide-gray-100 text-sm">
                                <div className="grid grid-cols-3 p-3">
                                    <dt className="text-text-muted font-bold text-xs pt-1">埋葬方法</dt>
                                    <dd className="col-span-2 text-text-primary font-medium">
                                        {plan.type === 'tree' ? '樹木葬（個別/合祀）' : plan.type === 'eitai' ? '合祀' : plan.type === 'noukotsudou' ? '納骨堂' : '一般墓'}
                                    </dd>
                                </div>
                                <div className="grid grid-cols-3 p-3">
                                    <dt className="text-text-muted font-bold text-xs pt-1">利用人数</dt>
                                    <dd className="col-span-2 text-text-primary font-medium">{plan.capacity || "要確認"}</dd>
                                </div>
                                <div className="grid grid-cols-3 p-3">
                                    <dt className="text-text-muted font-bold text-xs pt-1">使用期限</dt>
                                    <dd className="col-span-2 text-text-primary font-medium">
                                        {plan.type === 'general' ? '永代使用' : '13年〜33年（その後合祀）'}
                                    </dd>
                                </div>
                                <div className="grid grid-cols-3 p-3">
                                    <dt className="text-text-muted font-bold text-xs pt-1">ペット</dt>
                                    <dd className="col-span-2 text-text-primary font-medium">
                                        {plan.features.some(f => f.includes("ペット")) ? "共葬可" : "不可"}
                                    </dd>
                                </div>
                                <div className="grid grid-cols-3 p-3">
                                    <dt className="text-text-muted font-bold text-xs pt-1">備考</dt>
                                    <dd className="col-span-2 text-text-muted text-xs leading-relaxed">
                ...
                {/* Footer CTA */}
                <div className="p-6 bg-white border-t border-border mt-auto sticky bottom-0 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        ...
                        <div className="hidden lg:block w-full">
                            <a href="tel:0120-000-000" className="w-full">
                                <Button size="lg" variant="ghost" className="w-full text-text-secondary hover:text-primary hover:bg-bg font-bold h-12">
