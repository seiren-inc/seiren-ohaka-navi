...
export default async function InquiryList(props: { searchParams: Promise<{ category?: string }> }) {
    ...
    const getStatusBadge = (status: InquiryStatus) => {
        switch (status) {
            ...
            case 'done': return <span className="flex items-center gap-1 bg-bg text-text-secondary px-2 py-1 rounded text-xs font-bold"><CheckCircle2 className="w-3 h-3" /> 完了</span>;
        }
    };
    ...
    const getCategoryBadge = (inquiry: Inquiry) => {
        ...
        switch (cat) {
            ...
            default: return <span className="bg-bg text-text-secondary px-2 py-1 rounded text-xs">その他</span>;
        }
    }
    ...
    return (
        <div>
            <div className="flex justify-between items-end mb-6">
                <div>
                    <h2 className="text-2xl font-bold text-primary-dark">問い合わせ一覧</h2>
                    <p className="text-sm text-text-muted mt-1">全 {inquiries.length} 件</p>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-border overflow-hidden">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {inquiries.length === 0 && (
                            <tr>
                                <td colSpan={7} className="p-8 text-center text-text-muted">問い合わせはまだありません</td>
                            </tr>
                        )}
                        {inquiries.map(i => (
                            <tr key={i.id} className="hover:bg-bg text-sm group">
                                <td className="p-4 text-text-muted whitespace-nowrap">
                                    <Link href={`/admin/inquiries/${i.id}`} className="block hover:text-blue-600">
                                        <div className="font-bold text-text-primary mb-1">{getCategoryBadge(i)}</div>
                                        <div className="text-xs">{new Date(i.createdAt).toLocaleString('ja-JP')}</div>
                                        <div className="text-[10px] text-text-muted font-mono mt-0.5">ID: {i.id.slice(-4)}</div>
                                    </Link>
                                </td>
                                ...
                                <td className="p-4">
                                    {i.kind === 'business' ? (
                                        <div className="font-bold text-text-primary">
                                            <div className="text-xs text-text-muted mb-0.5">会社/寺院名</div>
                                        ...
                                        <>
                                            {i.category === 'grave_closure' ? (
                                                <div className="mb-2">
                                                    ...
                                                    {i.graveTemplePref && (
                                                        <span className="text-xs text-text-muted">({i.graveTemplePref})</span>
                                                    )}
                                                </div>
                                            ) : (
                                                <div className="font-bold text-text-primary">
                                                    <div className="text-xs text-text-muted mb-0.5">希望墓地</div>
                                            ...
                                            <div className="mt-2">
                                                ...
                                                {i.context?.planName || i.desiredPlanName || i.additionalFields?.hasNextPlace ? (
                                                    ...
                                                    <span className="text-sm text-text-muted">未指定</span>
                                                )}
                                            </div>
                                        </>
                                    )}
                                ...
                                <td className="p-4 text-xs max-w-[150px]">
                                    <div className="font-bold text-text-secondary">{i.context?.sourceLabel || i.context?.source || '-'}</div>
                                    <div className="text-text-muted truncate">{i.context?.sourcePath || i.ref || '-'}</div>
                                </td>
                                <td className="p-4 text-text-primary">
                                    <div className="font-bold">{i.user?.name || i.contactName || (i as any).name || '不明'}</div>
                                    <div className="text-text-muted text-xs">{i.user?.phone || (i as any).phone || '-'}</div>
                                </td>
                                <td className="p-4 max-w-[200px]">
                                    {i.kind === 'business' ? (
                                        <div className="text-xs">
                                            <span className="font-bold text-text-muted">種別:</span> {i.inquiryType}
                                        </div>
                                    ) : (
                                    ...
                                    <div className="truncate text-text-secondary mt-1" title={i.message}>{i.message}</div>
                                </td>
