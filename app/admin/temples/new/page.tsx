        ...
        <div className="space-y-8 max-w-4xl animate-in fade-in">
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-3">
                    {SECTS.map(s => (
                        <label key={s} className={`cursor-pointer px-4 py-2 rounded-full border transition-colors ${temple.sects.includes(s) ? 'bg-primary text-white border-primary' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-3">
                    {MEMORIAL_TYPES.map(m => (
                        <label key={m} className={`cursor-pointer px-4 py-2 rounded-full border transition-colors ${temple.supportedMemorialTypes.includes(m) ? 'bg-primary text-white border-primary' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
        ...
        return (
            <div className="space-y-12 max-w-4xl animate-in fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 h-full flex flex-col"><label className="block text-sm font-bold text-gray-700 mb-2 flex items-center gap-2"> <Settings className="w-4 h-4" /> 予約受付チャネル</label><div className="flex-1 flex gap-4 w-full"><label className="flex-1 flex items-center justify-center gap-3 cursor-pointer bg-white px-4 py-2 border-2 rounded-lg hover:bg-gray-50 transition-colors shadow-sm h-full"><input type="checkbox" checked={cal.bookingChannels.includes('form')} onChange={e => updateCal({ bookingChannels: e.target.checked ? [...cal.bookingChannels, 'form'] : cal.bookingChannels.filter(c => c !== 'form') })} className="w-5 h-5 text-primary rounded border-gray-300 accent-seiren-navy" /><span className="font-bold text-sm text-gray-800 whitespace-nowrap">Webフォーム</span></label><label className="flex-1 flex items-center justify-center gap-3 cursor-pointer bg-white px-4 py-2 border-2 rounded-lg hover:bg-gray-50 transition-colors shadow-sm h-full"><input type="checkbox" checked={cal.bookingChannels.includes('phone')} onChange={e => updateCal({ bookingChannels: e.target.checked ? [...cal.bookingChannels, 'phone'] : cal.bookingChannels.filter(c => c !== 'phone') })} className="w-5 h-5 text-primary rounded border-gray-300 accent-seiren-navy" /><span className="font-bold text-sm text-gray-800 whitespace-nowrap">電話</span></label></div></div>
                </div>
