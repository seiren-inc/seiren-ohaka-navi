        ...
        <Card hoverEffect className="p-0 overflow-hidden flex flex-col md:flex-row h-full transition-all duration-300 hover:shadow-md">
            ...
            {/* Badges: facility type + plan badges */}
                <div className="absolute top-2 left-2 flex flex-wrap gap-1">
                    <span className="bg-primary/90 text-white text-xs px-2 py-1 rounded-sm uppercase tracking-wider font-bold shadow-sm backdrop-blur-sm">
                        {data.type}
                    </span>
                    {data.status === 'private' && <span className="bg-gray-500 text-white text-xs px-2 py-1 rounded-sm">非公開</span>}
                </div>
                {/* PR / おすすめバッジ（右上） */}
