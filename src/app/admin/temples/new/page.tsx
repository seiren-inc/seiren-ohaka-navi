...
// New Enums Lists
const SECTS: Sect[] = ['無宗派', '仏教全般', '浄土宗', '浄土真宗', '日蓮宗', '真言宗', '天台宗', '曹洞宗', '禅宗', 'その他'];
...
export default function NewTemplePage() {
    ...
    const handleZipCode = async (zip: string) => {
        // state に保存
        setTemple(prev => ({ ...prev, ...{ zipCode: zip } } as any));
        ...
        <div className="space-y-8 max-w-4xl animate-in fade-in">
            ...
            <div>
                ...
                <textarea
                    ...
                    placeholder="例: 都営大江戸線「赤羽橋駅」より徒歩5分。東京駅よりバスで約15分。"
                    value={temple.access}
        ...
        <div className="space-y-8 max-w-4xl animate-in fade-in">
            ...
            <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                    {SECTS.map(s => (
                        <label key={s} className={`cursor-pointer px-4 py-2 rounded-full border transition-all text-sm font-bold ${temple.sects.includes(s) ? 'bg-primary text-white border-primary shadow-sm' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'}`}>
            ...
            <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                    {MEMORIAL_TYPES.map(m => (
                        <label key={m} className={`cursor-pointer px-4 py-2 rounded-full border transition-all text-sm font-bold ${temple.supportedMemorialTypes.includes(m) ? 'bg-primary text-white border-primary shadow-sm' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'}`}>
    ...
    const generateAI = async (type: 'catchphrase' | 'overview' | 'features') => {
        ...
        try {
            const res = await fetch('/api/ai/generate', {
                ...
                body: JSON.stringify({
                    ...
                    context: {
                        ...
                        cityName: temple.cityName,
                        type: temple.type,
                        sects: temple.sects as string[],
                        supportedMemorialTypes: temple.supportedMemorialTypes as string[],
                        tags: temple.tags as string[],
            ...
            if (type === 'catchphrase') {
                ...
                setTemple(prev => ({ ...prev, overview: data.result as string }));
            } else if (type === 'features') {
                const features = Array.isArray(data.result) ? data.result : [];
                if (features.length > 0) setTemple(prev => ({ ...prev, keyFeatures: features }));
            }
        } catch (err) {
        ...
        <div className="space-y-8 max-w-4xl animate-in fade-in">
            ...
            <div className="space-y-2">
                <div className="flex items-center justify-between">
                    ...
                    <button
                        type="button"
                        onClick={() => generateAI('catchphrase')}
                        disabled={aiLoading['catchphrase']}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r from-violet-500 to-indigo-500 text-white text-xs font-bold shadow hover:shadow-md transition-all disabled:opacity-60"
                    >
                        {aiLoading['catchphrase'] ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                        AI生成
                    </button>
                </div>
                <input
                    className="w-full border p-3 rounded-lg font-bold text-lg"
                    placeholder="例: 東京タワーを望む、都心の安らぎの聖地"
                    value={temple.catchphrase || ''}
                    onChange={e => setTemple({ ...temple, catchphrase: e.target.value })}
                />
                <div className="flex justify-end">
                    <span className={`text-xs font-bold ${(temple.catchphrase || '').length > 30 ? 'text-red-500' : 'text-gray-400'}`}>
                        {(temple.catchphrase || '').length} / 30文字
                    </span>
                </div>
                {/* AI候補 */}
                {catchphraseCandidates.length > 0 && (
                    <div className="bg-gradient-to-br from-violet-50 to-indigo-50 border border-violet-200 rounded-xl p-4">
                        ...
                        <div className="space-y-2">
                            {catchphraseCandidates.map((c, i) => (
                                <button
                                    key={i}
                                    type="button"
                                    onClick={() => { setTemple({ ...temple, catchphrase: c }); setCatchphraseCandidates([]); }}
                                    className="w-full text-left px-3 py-2 bg-white border border-violet-200 rounded-lg text-sm font-medium hover:bg-violet-50 hover:border-violet-400 transition-colors"
                                >
                                    {c}
                                </button>
                            ))}
            ...
            <div className="space-y-3">
                <div className="flex items-center justify-between">
                    ...
                    <button
                        type="button"
                        onClick={() => generateAI('features')}
                        disabled={aiLoading['features']}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r from-violet-500 to-indigo-500 text-white text-xs font-bold shadow hover:shadow-md transition-all disabled:opacity-60"
                    >
                        {aiLoading['features'] ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                        AI一括生成
                    </button>
                </div>
                <div className="space-y-4">
                    {[0, 1, 2].map(i => (
                        <div key={i} className="border rounded-xl p-4 bg-gray-50">
                            <div className="flex items-center gap-1 text-xs font-bold text-gray-500 mb-3">特徴 {i + 1}</div>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                <div>
                                    <label className="text-xs text-gray-500 block mb-1">アイコン</label>
                                    <select className="w-full border rounded-lg p-2 text-sm" value={temple.keyFeatures?.[i]?.icon} onChange={e => { const next = [...(temple.keyFeatures || [])]; if (!next[i]) next[i] = { title: '', text: '', icon: '駅近' }; next[i].icon = e.target.value as any; setTemple({ ...temple, keyFeatures: next }); }}>{CONTENT_ICONS.map(c => <option key={c} value={c}>{c}</option>)}</select>
                                </div>
                                <div>
                                    <label className="text-xs text-gray-500 block mb-1">タイトル（15文字以内）</label>
                                    <input className="w-full border rounded-lg p-2 text-sm" placeholder="例: 駅から徒歩5分" value={temple.keyFeatures?.[i]?.title || ''} onChange={e => { const next = [...(temple.keyFeatures || [])]; if (!next[i]) next[i] = { title: '', text: '', icon: '駅近' }; next[i].title = e.target.value; setTemple({ ...temple, keyFeatures: next }); }} />
                                </div>
                                <div>
                                    <label className="text-xs text-gray-500 block mb-1">説明（50文字以内）</label>
                                    <textarea className="w-full border rounded-lg p-2 text-sm h-16 resize-none" placeholder="例: バリアフリー対応で車椅子の方も安心" value={temple.keyFeatures?.[i]?.text || ''} onChange={e => { const next = [...(temple.keyFeatures || [])]; if (!next[i]) next[i] = { title: '', text: '', icon: '駅近' }; next[i].text = e.target.value; setTemple({ ...temple, keyFeatures: next }); }} />
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            <div>
                <div className="flex items-center justify-between mb-1">
                    ...
                    <button
                        type="button"
                        onClick={() => generateAI('overview')}
                        disabled={aiLoading['overview']}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r from-violet-500 to-indigo-500 text-white text-xs font-bold shadow hover:shadow-md transition-all disabled:opacity-60"
                    >
                        {aiLoading['overview'] ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                        AI生成
                    </button>
                </div>
                <textarea className="w-full border h-40 rounded-lg p-3 leading-relaxed" placeholder="施設の概要・特徴・雰囲気を入力..." value={temple.overview} onChange={e => setTemple({ ...temple, overview: e.target.value })} />
                <div className="flex justify-end mt-1">
                    <span className={`text-xs font-bold ${(temple.overview || '').length > 300 ? 'text-amber-500' : 'text-gray-400'}`}>{(temple.overview || '').length}文字</span>
                </div>
            </div>
                        ...
                        {(temple.suitableFor || ['', '', '']).map((v, i) => (
                            <div key={i} className="flex gap-2 items-center">
                                ...
                                <input
                                    ...
                                    placeholder={`例: ${['都心でお墓を探している方', '後継ぎがいない方', 'ペットと一緒に入りたい方'][i] || '...'}`}
                                    value={v}
                                    onChange={e => { const next = [...(temple.suitableFor || [])]; next[i] = e.target.value; setTemple({ ...temple, suitableFor: next }); }}
                                />
                    ...
                    <div className="space-y-2">
                        {(temple.notesPoints || ['', '', '']).map((v, i) => (
                            <div key={i} className="flex gap-2 items-center">
                                ...
                                <input
                                    className="flex-1 border rounded-lg p-2 text-sm"
                                    placeholder={`例: ${['ペット区画は数に限りがあります', '事前予約が必要です', '駐車場は近隣をご利用ください'][i] || '...'}`}
                                    value={v}
                                    onChange={e => { const next = [...(temple.notesPoints || [])]; next[i] = e.target.value; setTemple({ ...temple, notesPoints: next }); }}
                                />
    ...
    const renderPlanTab = () => (
        <div className="space-y-8 max-w-4xl animate-in fade-in">
            ...
            <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl p-12 text-center text-gray-500">
                ...
                <p className="font-bold text-lg mb-2 text-gray-700">区画プランの登録</p>
        ...
        const generateSEO = async () => {
            ...
            try {
                ...
                if (typeof data.result === 'object') {
                    updateSeo({
                        title: data.result.title || seo.title,
                        description: data.result.description || seo.description,
                        primaryKeywords: data.result.primaryKeywords || seo.primaryKeywords,
                        secondaryKeywords: data.result.secondaryKeywords || seo.secondaryKeywords,
                    });
                }
            } catch (err) {
                alert(`AI生成に失敗しました: ${err instanceof Error ? err.message : err}`);
            ...
            <div className="space-y-10 max-w-4xl animate-in fade-in">
                ...
                <div className="bg-gradient-to-r from-violet-50 to-indigo-50 border border-violet-200 rounded-2xl p-5 flex items-center justify-between">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-violet-500 to-indigo-500 text-white font-bold shadow hover:shadow-md transition-all disabled:opacity-60 shrink-0 ml-4"
                    >
                    ...
                    <div className="space-y-5">
                        <div>
                            ...
                            <div className="flex justify-between mt-1">
                                <p className="text-xs text-gray-500">32文字以内推奨</p>
                                <span className={`text-xs font-bold ${(seo.title || '').length > 32 ? 'text-red-500' : 'text-gray-400'}`}>{(seo.title || '').length}文字</span>
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="flex justify-between mt-1">
                                <p className="text-xs text-gray-500">120文字以内推奨</p>
                                <span className={`text-xs font-bold ${(seo.description || '').length > 120 ? 'text-red-500' : (seo.description || '').length > 100 ? 'text-amber-500' : 'text-gray-400'}`}>{(seo.description || '').length}文字</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
                        <div className="flex items-start gap-2"><HelpCircle className="w-5 h-5 text-yellow-600 mt-0.5" /><div className="text-sm text-yellow-800"><p className="font-bold mb-1">AI要約（Summary）の書き方ガイド</p><ol className="list-decimal list-inside space-y-1 ml-2"><li><span className="font-bold">結論</span>: この施設の最大の特徴（例：都心にある永代供養墓）</li><li><span className="font-bold">理由</span>: なぜおすすめか（例：アクセスが良く、バリアフリー）</li><li><span className="font-bold">補足</span>: 具体的な設備やプラン（例：ペット可、樹木葬あり）</li></ol></div></div>
                    </div>
                    <div>
                        ...
                        <textarea className="w-full border p-4 rounded-lg h-40 font-medium leading-relaxed" placeholder="例：〇〇霊園は、△△市にある自然豊かな公園墓地です。..." value={seo.summary || ''} onChange={e => updateSeo({ summary: e.target.value })} />
    ...
    return (
        <div className="max-w-6xl mx-auto pb-12" >
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-800">新規寺院登録</h2>
                <div className="flex gap-2">
